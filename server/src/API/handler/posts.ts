...
function Post(
	request: Request<{}, {}, Pick<Post, 'body' | 'affiliate_id'>>,
	response: Response,
	next: NextFunction
): void {
	if (
		request.body.body === undefined ||
		(request.body.body !== undefined && request.body.body.length === 0)
	) {
	...
	db.pool.getConnection((err, connection) => {
		...
		const payload = {
			...
			affiliates: [
				Number(response.locals.session.affiliate_id),
				Number(request.body.affiliate_id),
			]
				.filter(Boolean)
				.map((v) => ({ affiliate_id: v })),
		};
		...
		Controller.Posts.Create(connection, payload)
			.then((res) => {
				...
				response
					.status(201)
					.send({ created: true, payload: res.payload });
			})
			.catch(next);
	});
}
		...
		Controller.Posts.DeletePost(connection, { affiliate_id, post_id })
			.then((res) => {
				...
				if (!res.done) {
					response
						.status(400)
						.send({ deleted: false, message: res.message });
					return;
				}
		...
		Controller.Posts.SwitchSaved(connection, { affiliate_id, post_id })
			.then((res) => {
				...
				if (!res.done) {
					response
						.status(400)
						.send({ done: false, message: res.message });
					return;
				}
