...
function Unfollow(request: Request<Request['params'], {}, Pick<MemberFollowRequest, 'member_follow_request_id'>>, response: Response, next: NextFunction): void {
    if (request.body.member_follow_request_id === undefined) {
        response.status(400).send({ message: 'There\'s empty required fields' });
        return;
    }
    ...
    db.pool.getConnection((err, connection) => {
        ...
        const payload = {
            member_follow_request_id: Number(request.body.member_follow_request_id),
            to_affiliate_id: Number(request.params[0]),
        };
