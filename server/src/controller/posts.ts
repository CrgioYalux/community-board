...
interface PostOperation {
	...
	CreatePostMembership: {
		Action: (
			pool: PoolConnection,
			payload: Pick<Post, 'post_id'> & {
				affiliates: Array<Pick<Post, 'affiliate_id'>>;
			}
		) => Promise<
	...
	GetFromAffiliateID: {
		Action: (
			...
			payload: Pick<
				ViewAffiliatePosts,
				'consultant_affiliate_id' | 'post_membership_affiliate_id'
			>
		) => Promise<SelectQueryActionReturn<Array<ViewAffiliatePosts>>>;
		QueryReturnType: EffectlessQueryResult<ViewAffiliatePosts>;
	};
...
const CheckIfPostExists: PostOperation['CheckIfPostExists']['Action'] = (
	pool,
	payload
) => {
	return new Promise((resolve, reject) => {
		pool.query(
			PostOperationQuery.CheckIfPostExists,
			[payload.post_id],
			(err, results) => {
				if (err) {
				...
				const parsed =
					results as PostOperation['CheckIfPostExists']['QueryReturnType'];
...
const CreatePost: PostOperation['CreatePost']['Action'] = (pool, payload) => {
	return new Promise((resolve, reject) => {
		pool.query(
			PostOperationQuery.CreatePost,
			[payload.entity_id, payload.body],
			(err, results) => {
				if (err) {
				...
				const parsed =
					results as PostOperation['CreatePost']['QueryReturnType'];
...
const CreatePostMembership: PostOperation['CreatePostMembership']['Action'] = (
	pool,
	payload
) => {
	return new Promise((resolve, reject) => {
		const values = payload.affiliates.map((v) => [
			payload.post_id,
			v.affiliate_id,
		]);
		pool.query(
			PostOperationQuery.CreatePostMembership,
			[values],
			(err, results) => {
				if (err) {
				...
				const parsed =
					results as PostOperation['CreatePostMembership']['QueryReturnType'];

				if (!parsed.affectedRows) {
					resolve({
						done: false,
						message: 'Could not create the post membership',
					});
					return;
				}
	...
	return new Promise((resolve, reject) => {
		if (payload.affiliates.length === 0) {
			resolve({
				...
				message: 'Can not create the post without an affiliates list',
			});
			return;
		}
		...
		pool.beginTransaction((err0) => {
			...
			Common.CreateEntity(pool)
				.then((res1) => {
					...
					CreatePost(pool, {
						entity_id: res1.payload.entity_id,
						body: payload.body,
					})
						.then((res2) => {
							if (!res2.done) {
							...
							Common.CheckIfValidAffiliateByID(pool, {
								affiliate_id:
									payload.affiliates[0].affiliate_id,
							})
								.then((res3) => {
									if (!res3.found) {
										pool.rollback(() => {
									...
									if (
										!res3.payload.is_active &&
										res3.payload.is_member
									) {
										pool.rollback(() => {
											resolve({
												done: false,
									...
									if (
										!res3.payload.is_active &&
										res3.payload.is_board
									) {
										pool.rollback(() => {
											resolve({
												done: false,
												message: 'The board is deleted',
											});
										});
										return;
									}
									...
									if (
										...
										pool.rollback(() => {
											resolve({
												...
												message: 'The board is invalid',
											});
										});
										return;
									}

									const aux_affiliate_id =
										payload.affiliates[1]?.affiliate_id ??
										-1;

									Common.CheckIfValidAffiliateByID(pool, {
										affiliate_id: aux_affiliate_id,
									})
										.then((res4) => {
											if (aux_affiliate_id !== -1) {
												...
												if (
													res3.payload
														.affiliate_id ===
													res4.payload.affiliate_id
												) {
													pool.rollback(() => {
														resolve({
															done: false,
															message:
																'The affiliate IDs are the same',
														});
													});
													return;
												}
												...
												if (
													!res4.payload.is_active &&
													res4.payload.is_member
												) {
													pool.rollback(() => {
														resolve({
															done: false,
												...
												if (
													...
													pool.rollback(() => {
														resolve({
															...
															message:
																'Both affiliate IDs refer to members',
														});
													});
													return;
												}
												...
												if (
													...
													pool.rollback(() => {
														resolve({
															...
															message:
																'Both affiliate IDs refer to boards',
														});
													});
													return;
												}
											...
											CreatePostMembership(pool, {
												post_id: res2.payload.post_id,
													...
													pool.commit((err6) => {
														if (err6) {
															pool.rollback(
																() => {
																	reject({
																		createPostCommitTransactionError:
																			err6,
																	});
																}
															);
	...
	return new Promise((resolve, reject) => {
		pool.query(
			...
			(err, results) => {
				...
				const parsed =
					results as PostOperation['Get']['QueryReturnType'];
...
const GetFromAffiliateID: PostOperation['GetFromAffiliateID']['Action'] = (
	pool,
	payload
) => {
	return new Promise((resolve, reject) => {
		pool.query(
			PostOperationQuery.GetFromAffiliateID,
			[
				payload.consultant_affiliate_id,
				payload.post_membership_affiliate_id,
			],
			(err, results) => {
				if (err) {
					reject({ getError: err });
	...
	return new Promise((resolve, reject) => {
		pool.query(
			...
			(err, results) => {
				...
				const parsed =
					results as PostOperation['DeletePost']['QueryReturnType'];
	...
	return new Promise((resolve, reject) => {
		pool.query(
			...
			(err, results) => {
				...
				const parsed =
					results as PostOperation['SavePost']['QueryReturnType'];
				...
				.then((res1) => {
					...
					Common.CheckIfAlreadySavedByAffiliateID(pool, payload)
									...
									.then((res3) => {
										...
										pool.commit((err4) => {
											if (err4) {
												pool.rollback(() => {
													reject({
														switchSavedCommitTransactionError:
															err4,
													});
												});
												return;
						...
						.catch((err2) => {
							pool.rollback(() => {
								reject({
									checkIfAlreadySavedByAffiliateIDError: err2,
								});
							});
						});
				})
