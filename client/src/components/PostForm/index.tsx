    ...
    return (
        ...
        >
            <div className='flex flex-row gap-2 items-center'>
                <span className='text-xl font-bold'>{API.Value.member?.fullname}</span>
                <span 
                className='text-xs bg-gray-700 rounded-full px-2 py-1'
                >#{API.Value.member?.username}</span>
            </div>
            <textarea 
            ...
            <div className='flex flex-row justify-between'>
                <span className={`${isDisabled ? 'text-red-400' : 'text-current'} w-[8ch] font-semibold bg-gray-700 rounded-full px-2 grid place-items-center`}>{body.length}/255</span>
                <button 
