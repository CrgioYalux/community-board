		...
		Actions: {
			Auth: {
				Register: (
					...
					{ created: true } | { created: false; message: string }
				>;
				Login: (
			...
			Posts: {
				SwitchSave: (
					payload: APIAction.Posts.SwitchSave.Payload
				) => Promise<{ done: boolean }>;
				Create: (
					payload: APIAction.Posts.Create.Payload
				) => Promise<
					| { created: true; post: { post_id: number } }
					| { created: false; message: string }
				>;
				Delete: (
					payload: APIAction.Posts.Delete.Payload
				) => Promise<
			...
			Feed: {
				...
				>;
				GetFromAffiliateID: (
					payload: APIAction.Feed.GetFromAffiliateID.Payload
				) => Promise<
					| { found: true; posts: Post[] }
					| { found: false; message: string }
				>;
			...
			Members: {
				GetFromMemberPovByUsername: (
					payload: APIAction.Members.GetFromMemberPovByUsername.Payload
				) => Promise<
					| { found: true; member: MemberFromMemberPov }
					| { found: false; message: string }
				>;
				Edit: (
					payload: APIAction.Members.Edit.Payload
				) => Promise<{ done: true } | { done: false; message: string }>;
			};
			Followers: {
				GetRequests: () => Promise<
					| { found: true; requests: FollowRequest[] }
					| { found: false; message: string }
				>;
				AcceptRequest: (
					payload: APIAction.Followers.AcceptRequest.Payload
				) => Promise<{ done: true } | { done: false; message: string }>;
				DeclineRequest: (
					payload: APIAction.Followers.DeclineRequest.Payload
				) => Promise<{ done: true } | { done: false; message: string }>;
				Get: (
					payload: APIAction.Followers.Get.Payload
				) => Promise<
					| { found: true; followers: FollowRequest[] }
					| { found: false; message: string }
				>;
			};
			Followees: {
				Get: (
					...
					| { found: true; followees: FollowRequest[] }
					| { found: false; message: string }
				>;
			};
			Affiliates: {
				Follow: (
					payload: APIAction.Affiliates.Follow.Payload
				) => Promise<
					| { done: true; follow_request_id: number }
					| { done: false; message: string }
				>;
				Unfollow: (
					payload: APIAction.Affiliates.Unfollow.Payload
				) => Promise<{ done: true } | { done: false; message: string }>;
			};
		};
	}
