				...
				<div className="w-full h-full flex flex-col gap-2 bg-blue-900 dark:bg-blue-200 rounded">
					<div className="flex flex-row text-blue-200 dark:text-blue-900 p-2">
						<div className="flex flex-col font-semibold">
							<span className="text-xl w-[16ch] truncate">
								{API.Value.member?.fullname}
							</span>
							<Link
								to={`/members/${API.Value.member?.username}`}
								className="text-sm w-[14ch] truncate hover:underline cursor-pointer"
							>
					...
					<div className="flex flex-row gap-2 text-sm text-blue-200 dark:text-blue-900 px-2">
						<span className="tracking-wider">
							<Link
								to={`/affiliates/${API.Value.member?.affiliate_id}/followers`}
								className="hover:underline"
							>
								<span className="font-bold tracking-normal">
									{API.Value.member?.followers}
								</span>{' '}
								Followers
							</Link>
						</span>
						<span className="tracking-widest">
							<Link
								to={`/affiliates/${API.Value.member?.affiliate_id}/followees`}
								className="hover:underline"
							>
								<span className="font-bold tracking-normal">
									{API.Value.member?.followees}
								</span>{' '}
								Following
							</Link>
						</span>
