...
const MemberByUsername: React.FC = () => {
	...
	const fetchMember = (): void => {
		...
		API.Actions.Members.GetFromMemberPovByUsername({
			...
			.then((res) => {
				...
				API.Actions.Feed.GetFromAffiliateID({
					affiliate_id: res.member.affiliate_id,
				})
					.then((res1) => {
	...
	if ((loadingMember || API.Value.fetching) && !loadingPosts) {
		return (
			<div className="grow shrink-0 basis-full h-full flex flex-col items-center justify-center">
	...
	if (params.username === undefined || member === null) return <NotFound />;
	...
	const hasStrangerAcceptedClientRequest =
		hasClientSentRequest && member.is_consultant_allowed;
	const unfollowButtonVisible =
		isStranger && hasClientSentRequest && hasStrangerAcceptedClientRequest;
	const sentButtonVisible =
		isStranger && hasClientSentRequest && !hasStrangerAcceptedClientRequest;
			...
			.then((res) => {
				...
				setMember((prev) => {
					...
					return {
						...prev,
						followers:
							prev.followers === 0 ? 0 : prev.followers - 1,
			...
			<div className="flex flex-col gap-3 bg-blue-300 rounded p-4 text-gray-800">
				...
				<div className="flex flex-row gap-2 text-sm">
					<span className="tracking-wider">
						<Link
							to={`/affiliates/${member.affiliate_id}/followers`}
					...
					<span className="tracking-widest">
						<Link
							to={`/affiliates/${member.affiliate_id}/followees`}
							className="hover:underline"
						>
							<span className="font-bold tracking-normal">
								{member.followees}
				...
				<div className="flex flex-row justify-between text-sm">
					<div className="flex flex-row gap-2 items-center font-semibold">
						...
						{member.birthdate === null
							? '---'
							: new Date(member.birthdate).toDateString()}
					</div>
					<div className="flex flex-row gap-2 items-center font-semibold">
